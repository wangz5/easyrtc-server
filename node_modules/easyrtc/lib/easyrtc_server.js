...
exports.listen = function(httpApp, socketServer, options, listenCallback) {
    ...
    pub.util.logInfo("Started Server ( EasyRTC v" + pub.getVersion() +" ) on NodeJS (" + process.version + ")");
    pub.events.emit("startup", function(err) {
        if (err) {
            ...
            pub.util.logDebug("EasyRTC Server Ready For Connections (v"+ pub.getVersion() + ")");
            if(_.isFunction(listenCallback)) {
                listenCallback(err, pub);
            }
        }
    });
    pub.events.emit("started");
};
