...
eventListener.onLog = function(level, logText, logFields, next) {
    ...
    if (pub.getOption("logColorEnable")) {
        ...
        if(pub.getOption("logDateEnable")) {
            consoleText +=  " - "+g.dateToString(currentDate).grey;
        }
        consoleText += " : ".bold + logText;
    }
    else {
        if(pub.getOption("logDateEnable")) {
            consoleText += g.dateToString(currentDate) + " - ";
        }
        consoleText += level;
        consoleText += " : " + logText;
    }
...
eventListener.onZqnbEvent = function(next){
	pub.util.logWarning("Handler default customer event: onZqnbEvent.");
	next(null);
};
